> **Proposal record.** This file is the evidence-gated architecture from PR #2 (2026-09-03), kept verbatim. Revision 3 of `docs/klon-handoff.md` adopted from it: Git-owned worktree records, the transaction journal with `doctor` repair, approval of repository-supplied commands, the benchmark manifest rules, the manifest-equality backend test, and the support floor. Revision 3 deferred: content-bound receipts with execution manifests, `verify`, SQLite, and the immutable generation as the only clone source. See handoff §2 for each decision and its reason.
